...
/**
 * Founders Waitlist capture. Persists to Supabase + kicks off the drip via
 * /api/waitlist, and carries the business context (idea/niche/persona) the
 * visitor was exploring so the welcome email can be personalised.
 */
export function WaitlistForm({
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      track("waitlist_joined", { source: source ?? "", niche: niche ?? "" });
      setStatus("done");
    } catch {
      setStatus("error");
    }
  ...
  if (done) {
    return (
      ...
      >
        <Check className="h-4 w-4 shrink-0" /> You&apos;re on the list — check your inbox to get started.
      </div>
    );
  }
    ...
    <div className={cn("w-full max-w-md", className)}>
      <form
        ...
        className={cn(
          ...
          invert ? "border-white/15 bg-white/5" : "border-ink-200 bg-white",
        )}
      >
        <input
          type="email"
