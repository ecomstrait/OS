...
/** A lane of the commerce journey — drives the /services page's category tabs. */
export type ServiceCategory = "build" | "sell" | "automate" | "grow";
...
export type Service = {
  ...
  description: string;
  features: string[];
  /** Single source of truth for which category tab this shows under — was
   *  previously duplicated into a separate title-keyed lookup table in
   *  services-explorer.tsx that silently fell out of sync with these titles
   *  (every "Sell"/"Grow" service disappeared from its tab as a result). */
