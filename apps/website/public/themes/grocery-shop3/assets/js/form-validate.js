	...
	//}else if( f.phone.value == '' ){
		...
		notice( f.message );
		 ...
		 jQuery.ajax({
			url: 'mail.php',
			type: 'post',
			data: jQuery('form#ecomstrait-contact').serialize(),
			complete: function(data) {
				jQuery('#form_status').html(data.responseText);
				jQuery('#ecomstrait-contact').find('input,textarea').attr({value:''});
				jQuery('#ecomstrait-contact').css({opacity:1});
				jQuery('#ecomstrait-contact').remove();
			}
		});
		jQuery('#form_status').html('<span class="loading">Sending your message...</span>');
		jQuery('#ecomstrait-contact').animate({opacity:0.3});
		jQuery('#ecomstrait-contact').find('input,textarea,button').css('border','none').attr({'disabled':''});
	}
...
function notice( f ){
	jQuery('#ecomstrait-contact').find('input,textarea').css('border','none');
